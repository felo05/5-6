...
class StaffScreenState extends State<StaffScreen> {
      ...
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: SizedBox(
              width: (headers.length + fixedColumns) * 150.0,
              child: GridView.builder(
                shrinkWrap: true,
                physics: const NeverScrollableScrollPhysics(),
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                  crossAxisCount: headers.length + fixedColumns,
                  childAspectRatio: 2.5,
                ),
                itemCount: (staffList.length + 1) * (headers.length + fixedColumns),
  ...
  Widget _buildAttendanceCell(int index) {
    final totalColumns = headers.length + (selectedValue == "all" ? 6 : selectedValue == "tsme3" ? 2 : 3);
    ...
    if (row == 0) {
      ...
      if (col == 0) {
        ...
        else if (col == 5) value = staffList[row - 1].count3shea.toString();
        else value = staffList[row - 1].attended.contains(headers[col - 6]) ? '✓' : '';
      } else if (selectedValue != "tsme3") {
        if (col == 2) {
          switch (selectedValue) {
            case "2das":
